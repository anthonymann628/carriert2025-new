...
public class VoiceAssistant{
	...
	public void flushQueue(){
		...
		int error = textToSpeech.speak("", TextToSpeech.QUEUE_FLUSH, engineParams, utteranceId);
		if(error != -1){
			Log.e(TAG, "Error when flushing Android's TextToSpeech: " + error);
		}
	}
	...
	public void speak(String speechMessage){
		...
		int error = textToSpeech.speak(speechMessage, TextToSpeech.QUEUE_ADD, engineParams, utteranceId);
		if(error != -1){
